...
const Stage3 =()=>{
    ...
    const handleUpload = async () => {
      ...
      if(file === null){
        alert("אתה חייב להעלות קובץ קודם");
      }
      else{
        try {
          ...
          formData.append('id', sessionStorage.getItem("proid"));
               ...
               <div className='zain'>
                    <div className="align-self-center card login__card shadow-sm w-100">
                        <div className="card-body">
                            <h4 className="text-center">  צילום תעודת זהות</h4>
                            <input className='button btn-primary' type="file" width={200} onChange={handleChange} /><br /><br />
                            <button className='button btn-primary' onClick={handleUpload}>העלה תמונה</button>
                        </div>
                    </div>
                </div>
                </div>
